class AlterationNotesController < ApplicationController
  ...
  def new
    @alteration_note = AlterationNote.new
  end
  ...
  def create
    @alteration_note = AlterationNote.new(alteration_note_params)

    if @alteration_note.save
      redirect_to @alteration_note, notice: "Alteration note was successfully created."
    else
      render :new
    end
  end
  ...
  def update
    if @alteration_note.update(alteration_note_params)
      redirect_to @alteration_note, notice: "Alteration note was successfully updated."
    else
      render :edit
    end
  end
  ...
  def destroy
    ...
    redirect_to alteration_notes_url, notice: "Alteration note was successfully destroyed."
  end
  ...
  def set_alteration_note
    @alteration_note = AlterationNote.find(params[:id])
  end

  # Only allow a trusted parameter "white list" through.
  def alteration_note_params
    params.require(:alteration_note).permit(:alteration_id, :user_id)
  end
end
